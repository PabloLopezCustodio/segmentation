...
class ImageListener:
    ...
    def __init__(self, network, network_crop):
        ...
        if cfg.TEST.ROS_CAMERA == 'D415':
            ...
            rgb_sub = message_filters.Subscriber('/camera/color/image_rect_color', Image, queue_size=10)
    ...
    def run_network(self):
        ...
        out_label = test_sample(sample, self.network, self.network_crop)
        ...
        im_label_original = visualize_segmentation(im_color[:, :, (2, 1, 0)], label, return_rgb=True)
        im_label = cv2.cvtColor(im_label_original, cv2.COLOR_BGR2GRAY)
        # (1280*2/5, 720*2/5) 
        im_label = cv2.resize(im_label, (512, 288), interpolation = cv2.INTER_AREA)
        rgb_msg = self.cv_bridge.cv2_to_imgmsg(im_label, 'mono8')
